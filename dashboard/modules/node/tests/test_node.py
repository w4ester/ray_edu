...
def test_nodes_update(enable_test_module, ray_start_with_dashboard):
    ...
    while True:
        time.sleep(1)
        try:
            response = requests.get(webui_url + "/test/dump", timeout=60)
            ...
            response = requests.get(webui_url + "/test/notified_agents", timeout=60)
            response.raise_for_status()
            try:
...
def test_node_info(disable_aiohttp_cache, ray_start_with_dashboard):
    ...
    while True:
        time.sleep(1)
        try:
            response = requests.get(webui_url + "/nodes?view=hostnamelist", timeout=60)
            response.raise_for_status()
            ...
            response = requests.get(webui_url + f"/nodes/{node_id}", timeout=60)
            response.raise_for_status()
    ...
    def _check_nodes():
        try:
            ...
            for node_info in summary:
                ...
                detail = detail["data"]["detail"]
                assert detail["raylet"]["state"] == "ALIVE"
            response = requests.get(webui_url + "/test/dump?key=agents", timeout=60)
            response.raise_for_status()
...
def test_multi_node_churn(
    ...
    def get_nodes():
        resp = requests.get(webui_url + "/nodes?view=summary", timeout=60)
        resp.raise_for_status()
        summary = resp.json()
        assert summary["result"] is True, summary["msg"]
