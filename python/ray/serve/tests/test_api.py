...
def test_e2e(serve_instance):
    ...
    resp = requests.get("http://127.0.0.1:8000/api", timeout=60).json()["method"]
    assert resp == "GET"

    resp = requests.post("http://127.0.0.1:8000/api", timeout=60).json()["method"]
    assert resp == "POST"
...
def test_starlette_response_basic(serve_instance):
    ...
    assert requests.get("http://127.0.0.1:8000/", timeout=60).text == "Hello, world!"
...
def test_starlette_response_html(serve_instance):
    ...
    serve.run(html.bind())
    assert (
        requests.get("http://127.0.0.1:8000/", timeout=60).text
        == "<html><body><h1>Hello, world!</h1></body></html>"
    )
...
def test_starlette_response_json(serve_instance):
    ...
    serve.run(json.bind())
    assert requests.get("http://127.0.0.1:8000/json", timeout=60).json()["hello"] == "world"
...
def test_starlette_response_redirect(serve_instance):
    ...
    assert requests.get("http://127.0.0.1:8000/redirect", timeout=60).text == "Hello, world!"
...
def test_deploy_function_no_params(serve_instance, use_async):
    ...
    assert (
        requests.get(f"http://localhost:8000/{deployment_cls.name}", timeout=60).text
        == expected_output
    )
...
def test_deploy_class_no_params(serve_instance, use_async):
    ...
    assert requests.get(f"http://127.0.0.1:8000/{deployment_cls.name}", timeout=60).json() == {
        "count": 1
    }
    assert requests.get(f"http://127.0.0.1:8000/{deployment_cls.name}", timeout=60).json() == {
        "count": 2
...
def test_scaling_replicas(serve_instance):
    ...
    counter_result = []
    for _ in range(10):
        resp = requests.get("http://127.0.0.1:8000/counter", timeout=60).json()
...
def test_starlette_request(serve_instance):
    ...
    resp = requests.post("http://127.0.0.1:8000/api", data=long_string, timeout=60).text
...
def test_deploy_application_basic(serve_instance):
    ...
    assert requests.get("http://127.0.0.1:8000/", timeout=60).text == "got f"
    ...
    assert requests.get("http://127.0.0.1:8000/app_g", timeout=60).text == "got g"
    ...
    assert requests.get("http://127.0.0.1:8000/my_prefix", timeout=60).text == "got h"

    # Test FastAPI
    serve.run(MyFastAPIDeployment.bind(), name="FastAPI")
    assert requests.get("http://127.0.0.1:8000/hello", timeout=60).text == '"Hello, world!"'
...
def test_delete_application(serve_instance):
    ...
    serve.delete("app_f")
    assert "Path '/' not found" in requests.get("http://127.0.0.1:8000/", timeout=60).text
...
def test_deploy_application_with_same_name(serve_instance):
    ...
    assert requests.get("http://127.0.0.1:8000/", timeout=60).text == "got model"
    deployment_info = ray.get(controller._all_running_replicas.remote())
    ...
    assert requests.get("http://127.0.0.1:8000/", timeout=60).text == "got model1"
    deployment_info = ray.get(controller._all_running_replicas.remote())
    ...
    assert requests.get("http://127.0.0.1:8000/my_app", timeout=60).text == "got model1"
    assert requests.get("http://127.0.0.1:8000/", timeout=60).status_code == 404
...
def test_deploy_application_with_route_prefix_conflict(serve_instance):
    ...
    assert handle.remote().result() == "got model1"
    assert requests.get("http://127.0.0.1:8000/model1", timeout=60).text == "got model1"
...
def test_application_route_prefix_override(serve_instance, ingress_route, app_route):
    ...
    if app_route is None:
        ...
        assert requests.get(f"http://localhost:8000{app_route}", timeout=60).text == "hello"
...
def test_application_route_prefix_override1(serve_instance, ingress_route):
    ...
    if ingress_route is None:
        ...
        assert requests.get(f"http://localhost:8000{ingress_route}", timeout=60).text == "hello"
