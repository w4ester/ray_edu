    ...
    def __init__(
        ...
        try:
            while state in ["PENDING", "RUNNING"]:
                ...
                response = safe_requests.get(urljoin(url_base, statement_id) + "/",
                    auth=req_auth,
                    headers=req_headers,
                )
                response.raise_for_status()
        ...
        def get_read_task(task_index, parallelism):
            ...
            def _read_fn():
                for chunk_index in chunk_index_list:
                    ...
                    resolve_response = safe_requests.get(resolve_external_link_url, auth=req_auth, headers=req_headers
                    )
